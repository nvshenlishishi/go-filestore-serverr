...
```
- 登录并进入UI首页(http://<server_addr>:18080)
- 创建一个user, 用于管理存储对象
``` 
#(程序里的sdk通过access_key及secret_key可进行对象的操作,上传下载等)
docker exec ceph-rgw radosgw-admin user create --uid=test_user --display-name=test_user --system
{
    ...
    "keys": [
        {
            "user": "test_user",
            "access_key": "PFEA7NXWXSOWVTFA16C9",
            "secret_key": "xxxx"
        }
    ],
    "swift_keys": [],
...
```
-  http://docs.ceph.com/docs/nautilus/mgr/dashboard/